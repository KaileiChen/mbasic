...
#' @importFrom gtools rdirichlet
#' @export
MBASIC.sim.state <- function(I, K, J, S = 2, struct = NULL, delta = NULL, delta.non = NULL, zeta = 0.1, statemap = NULL, condition = TRUE) {
...
#  ProbMat[ , non.id ] <- c(t(rdirichlet(K * length(non.id), delta.non)))
  ...
  if (condition == FALSE){
    for(id in non.id) {
      ProbMat[ , id ] <- rep(rdirichlet(1, delta.non), K)
    }
  } else {
      ProbMat[ , non.id ] <- c(t(rdirichlet(K * length(non.id), delta.non)))
  }
...
#' @importFrom msm rtnorm
#' @export
MBASIC.sim <- function(xi, family = "lognormal", struct = NULL, I, fac, J, S = 2, f = 5, delta = NULL, delta.non = NULL, zeta = 0.1, condition = TRUE, statemap = NULL) {
  
  if(!family %in% c("lognormal1", "lognormal2", "negbin", "binom", "poisson")) {
    stop("Error: 'family' must be one of 'lognormal', 'negbin', 'binom' or 'poisson'.")
  }
  ...
  para.theta <- MBASIC.sim.state(I=I, K=K, J=J, S=S, delta=delta, delta.non=delta.non, zeta=zeta, struct = struct, condition = condition, statemap = statemap)
  ...
  if(family != "binom") {  
    ...
    prior.Mu <- t(matrix(rbeta(M * N, f * (2 * seq(M) - 1), f *(2* rev(seq(M)) - 1)), nrow = M))
  }

  if (family == "poisson"){
	prior.Mu[,1] = pmax(prior.Mu[,1], sqrt(2.0))
  }
  ...
  X <- NULL
  if(family == "loglognormal" | family == "lognormal") {
	...
	Y <- matrix(rpois(N * I, lambda = Lambda), nrow = N)
  ...
  if (family != "poisson"){
	...
	return(list(Theta = para.theta$Theta, Y = Y, X = X, fac = fac, W = para.theta$W, Z = para.theta$Z, V = V, delta = para.theta$delta, zeta = para.theta$zeta, prior.mean = prior_mean, prior.sd = prior_sd, stdev = NULL, Mu = prior.Mu, Lambda = prior.Mu*prior.Mu, bkng = bkng, snr = snr, non.id = para.theta$non.id))
  }  
}
